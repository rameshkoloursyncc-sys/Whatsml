...
import FindMessageModal from './FindMessageModal.vue'
...
const assignBadgeToChat = () => {
  assignBadgeForm.put(route('user.conversations.badge.assign', chatStore.activeConversationId), {
    onSuccess: () => {
...
const addNewBadge = () => {
  badgeForm.post(route('user.badges.store'), {
    onSuccess: (page) => {
      ...
      badgeForm.reset()
      chatStore.badges = page.props.badges
    }
  })
}
// ------------> badge end <-------------
...
<template>
  ...
  <slot name="dynamic-template-modal" />
  ...
  <Modal :header-state="true" header-title="Assign Badge" state="addBadgeModal">
    <form @submit.prevent="assignBadgeToChat">
      <div class="mb-3 flex items-center justify-between gap-2">
        <select class="select" v-model="assignBadgeForm.badge_id">
  ...
  <Modal @close="selectedAiTemplate = null" modal-type="sidebar" :modal-size="selectedAiTemplate ? 'w-4/12' : 'w-3/12'"
    :header-state="true" header-title="AI Tools" state="aiModal">
        ...
        <div class="card card-body">
          <div class="flex items-center justify-start gap-4 border-b border-primary-600 pb-4">
            <button type="button" @click="setAiTemplate(temp)" class="logo h-10 w-10">
              <img v-lazy="uiAvatar(temp.title, temp.icon)" alt="image" class="h-full w-full rounded-full" />
            </button>
            ...
            <button @click="bookmark(temp.id, temp.isBookmarked)" type="button" class="btn ml-auto h-8 w-8 rounded-full"
              :class="{
      ...
      <form @submit.prevent="aiStore.submit" class="space-y-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            <img :src="uiAvatar(selectedAiTemplate.title, selectedAiTemplate.icon)" class="h-10 w-10 rounded-full"
              alt="icon" />
            <div class="mx-2">
              <p class="text-sm leading-3">
                {{ selectedAiTemplate.title }}
              </p>
          ...
          <button class="btn btn-outline-danger btn-xs" type="button" @click="
            () => {
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            <Multiselect mode="single" class="multiselect-dark" v-model="aiStore.form.language"
              :options="chatStore.languages" valueProp="name" input-label="name" placeholder="Language" />
            <InputFieldError v-if="aiStore.errors?.language" :message="aiStore.errors?.language[0]" />
          </div>
          <div>
            <label class="label mb-1">{{ trans('Tone') }}*</label>
            <Multiselect mode="single" class="multiselect-dark" v-model="aiStore.form.tone" :options="aiStore.tones"
              value-prop="" placeholder="Tone" />
        ...
        <div>
          <div v-for="(field, i) in aiStore.form.fields" :key="i">
            ...
            <input required v-if="field.type === 'input'" v-model="field.value" type="text" class="input"
              :placeholder="field.placeholder" />
            <textarea v-if="field.type === 'textarea'" required v-model="field.value" class="textarea"
              :placeholder="field.placeholder"></textarea>
            <InputFieldError v-if="aiStore.hasError" :message="aiStore.fieldErrors[field.name]" />
            <InputFieldError v-if="!aiStore.hasError && aiStore.errors && aiStore.errors['fields.' + i + '.value']"
        ...
        <div class="flex items-center gap-5">
          <div class="flex-1">
            ...
            <input v-model.number="aiStore.form.max_token" type="text" class="input h-10"
              placeholder="Maximum Length" />
            <InputFieldError v-if="aiStore.errors?.max_token" :message="aiStore.errors?.max_token[0]" />
          </div>
          <div class="flex-1">
            <label class="label mb-1">{{ trans('Creativity Level') }}</label>
            <Multiselect mode="single" class="multiselect-dark" v-model="aiStore.form.creativity"
              :options="aiStore.creativityLevels" value-prop="value" inputLabel="label"
              placeholder="Creativity Level" />
            <InputFieldError v-if="aiStore.errors?.creativity" :message="aiStore.errors.creativity[0]" />
          </div>
        </div>

        <SpinnerBtn classes="btn btn-primary w-full py-3" :processing="aiStore.isProcessing"
          :btn-text="trans('Generate')" />
      </form>
      <div class="mt-4 flex flex-col gap-y-3">
        <div class="flex items-center gap-x-2">
          <input v-model="aiStore.documentName" type="text" class="input" placeholder="Untitled Document..." />
          <div class="flex">
            <button class="ms-1" @click="aiStore.downloadHTMLFile(aiStore.generatedText, aiStore.documentName)">
        ...
        <div class="flex justify-end gap-2" v-if="aiStore.generatedText">
          <button class="btn btn-info btn-sm" type="button" @click="inputMessage.message = aiStore.generatedText">
            <Icon icon="bx:pen" class="size-4" />
            <span>{{ trans('Use') }}</span>
          </button>
          <button class="btn btn-primary btn-sm" type="button" @click="copyToClipboard(aiStore.generatedText)">
