<script setup>
import moment from 'moment'

import Paginate from '@/Components/Dashboard/Paginate.vue'
import NoDataFound from '@/Components/NoDataFound.vue'
import sharedComposable from '@/Composables/sharedComposable'

import AdminLayout from '@/Layouts/Admin/AdminLayout.vue'

defineOptions({ layout: AdminLayout })
const props = defineProps(['campaigns'])
const { deleteRow } = sharedComposable()
</script>

<template>
  <div class="table-responsive mt-6 w-full">
    <table class="table">
      <thead>
        <tr>
          <th>
            {{ trans('Campaign') }}
          </th>
          <th>{{ trans('Group') }}</th>
          <th>
            {{ trans('Type') }}
          </th>
          <th>{{ trans('Schedule') }}</th>
          <th>{{ trans('Owner') }}</th>
          <th>{{ trans('Status') }}</th>
          <th>{{ trans('Created At') }}</th>
          <th class="!text-right">
            {{ trans('Action') }}
          </th>
        </tr>
      </thead>
      <tbody v-if="campaigns.data.length" class="tbody">
        <tr v-for="(campaign, index) in campaigns.data" :key="index">
          <td>
            {{ campaign.name }}
          </td>
          <td>
            {{ campaign?.group?.name }}
          </td>
          <td>
            {{ campaign.message_type }}
          </td>
          <td>
            <div class="flex flex-col gap-1">
              <span
                class="font-base text-[11px]"
                v-if="campaign.send_type == 'scheduled' && campaign.schedule_at"
              >
                <span class="font-bold">
                  {{ campaign.meta?.schedule_timezone }}
                </span>
                <br />
                {{ moment(campaign.schedule_timestamp).format('DD MMM, YY | h:mm A') }}
              </span>
              <span v-else>-</span>
            </div>
          </td>
          <td>
            <Link :href="route('admin.users.show', campaign.owner)">
              {{ campaign.owner?.name }}
            </Link>
          </td>
          <td>
            <span
              class="badge capitalize"
              :class="{
                'badge-info': campaign.status === 'draft',
                'badge-secondary': campaign.status === 'pending',
                'badge-primary': campaign.status === 'scheduled',
                'badge-success': campaign.status === 'send',
                'badge-danger': campaign.status === 'failed'
              }"
            >
              {{ campaign.status }}
            </span>
          </td>
          <td>
            {{ moment(campaign.created_at).format('dd-MM-YYYY hh:mm a') }}
          </td>
          <td>
            <div class="flex justify-end">
              <div class="dropdown" data-placement="bottom-start">
                <div class="dropdown-toggle">
                  <Icon class="h-5 text-3xl text-slate-400" icon="bx:dots-vertical-rounded" />
                </div>
                <div class="dropdown-content w-56">
                  <ul class="dropdown-list">
                    <li class="dropdown-list-item">
                      <button
                        class="dropdown-link delete-confirm"
                        href="#"
                        @click="deleteRow(route('admin.logs.campaigns.destroy', campaign))"
                      >
                        <Icon icon="bx:trash" />
                        {{ trans('Delete') }}
                      </button>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </td>
        </tr>
      </tbody>
      <NoDataFound :forTable="true" v-else />
    </table>

    <Paginate :links="campaigns.links" />
  </div>
</template>
