  ...
  <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
      <div class="card card-body mb-6">
        <div class="mb-4 flex items-center justify-between">
          <h2 class="text-xl font-semibold">
            {{ trans('Modules Update') }} - {{ module_json.name }}
          </h2>
          <p>{{ trans('Current Version') }}: {{ module_json.version }}</p>
        </div>
        ...
        <button @click="checkUpdate" :disabled="isLoading" class="btn btn-primary">
          <span v-if="!isLoading">{{ trans('Check New Update') }}</span>
      ...
      <div class="alert alert-info mb-6" role="alert">
        <p class="font-bold">{{ trans('Note') }}</p>
        <p>
          {{
            trans(`If you have customized the script from codebase, do not use this option. You will lose
          your customization.`)
          }}
        </p>
      </div>
      ...
      <div v-if="updateData && getUpdateData" class="card card-body">
        ...
        <div class="flex items-center justify-between">
          ...
          <button @click="confirmUpdate" :disabled="isLoading" class="btn btn-primary">
            <span v-if="!isLoading">{{ trans('Update now') }}</span>
            <span v-else class="inline-flex items-center">
              <svg
                class="-ml-1 mr-3 h-5 w-5 animate-spin text-white"
                xmlns="http://www.w3.org/2000/svg"
        ...
        <p v-if="getUpdateData.message" class="alert alert-info mt-4">
          {{ trans('Note') }}: {{ getUpdateData.message }}
        </p>
      </div>
    </div>
  </div>
