...
use App\Contacts\GatewayContact;

class SslCommerz implements GatewayContact
{
    protected $data = [];
    protected $config = [];
    ...
    public function __construct()
    {
        ...
        $sslGateway = Gateway::where('namespace', 'App\Gateway\SslCommerz')->firstOrFail();
        $sslGatewayConfig = json_decode($sslGateway->data, true);

        $apiDomain = $sslGateway->test_mode ? "https://sandbox.sslcommerz.com" : "https://securepay.sslcommerz.com";

        $this->config = [
            'apiCredentials' => [
                'store_id' => $sslGatewayConfig['store_id'],
                'store_password' => $sslGatewayConfig['store_password'],
            ],
            'apiUrl' => [
            ...
            'apiDomain' => $apiDomain,
            'connect_from_localhost' => boolval($sslGateway->test_mode), // For Sandbox, use "true", For Live, use "false"
    ...
    public static function make_payment($data)
    {
        $sslc = new self();

        $post_data = array();
        $post_data['total_amount'] = $data['pay_amount']; # You cant not pay less than 10
        ...
        $post_data['ship_name'] = $data['billName'];
        ...
        # OPTIONAL PARAMETERS
        $post_data['value_a'] = session('call_back.success');
        $post_data['value_b'] = session('call_back.fail');
        $post_data['value_c'] = session('plan_id');
    ...
    {
        // resolve payment and do action to database
    }
     ...
     */
    public function makePayment(array $requestData, $type = 'hosted', $pattern = 'json')
    {
        ...
        if ($type == 'hosted') {
            if (!empty($formattedResponse['GatewayPageURL'])) {
                return Inertia::location($formattedResponse['GatewayPageURL']);
            }
