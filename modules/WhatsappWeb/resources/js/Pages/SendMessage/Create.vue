...
const submitForm = () => {
  form.post(route('user.whatsapp-web.send-message.store'), {
    onSuccess: () => {
      form.reset()
      activeTab.value = 'text'
      setTemplateForm('text')
    }
  })
}
...
setTemplateForm(activeTab.value)
    ...
    <div class="card card-body md:col-span-9">
      <form @submit.prevent="submitForm" class="space-y-2">
        <div class="flex gap-3">
          <div class="w-full">
            <label class="label mb-1 capitalize"> {{ trans('Select Device') }}</label>
            <select name="platform_id" v-model="form.platform_id" class="select">
              <option value="" disabled selected>{{ trans('Select Platform') }}</option>
              <option v-for="device in platforms" :key="device.id" :value="device.uuid">
                {{ device.name }}
              </option>
            </select>
          </div>
          <InputField v-model="form.receiver_number" label="Message To (Receiver)" />
        </div>
        ...
        <template v-for="(value, field) in form.meta" :key="field">
          ...
          <div v-else-if="getFieldType(field, value) === 'checkbox'">
            <label class="label mb-1 capitalize">
              <input type="checkbox" v-model="form.meta[field]" class="checkbox mr-2" />
              {{ field }}
            </label>
          </div>
        ...
        <InputField v-if="form.save_as_template" v-model="form.name" label="Name" />
        ...
        <div class="flex items-center justify-between">
          <label for="save_as_template">
            <input
              ...
              class="checkbox"
            />
            <span class="ml-2">{{ trans('Save as template?') }}</span>
          </label>
          <SpinnerBtn
            btn-text="Submit"
            type="submit"
